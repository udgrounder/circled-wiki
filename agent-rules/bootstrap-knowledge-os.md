...
- 사용자 확인 뒤 `--apply`로 `.knowledge-os/` 아래 누락된 규칙·템플릿·정책·스키마·Portable CLI Runtime 생성
...
- 기존 `.knowledge-os/` 변경 전에 `.knowledge-os-backups/<기존-version>-<UTC timestamp>/`로 전체 스냅샷 생성
- 이전 설치 manifest checksum과 일치하는 관리 파일만 업그레이드
- 충돌한 관리 파일의 새 버전을 `.knowledge-os/proposals/`에 별도 제안본으로 생성
...
- `.knowledge-os/bin/knowledge-os.py validate` 실행 가능 여부와 Agent Bootstrap 문서 존재
- `.knowledge-os/issues/README.md`와 `system-observation` Profile 존재; 기존 이슈 기록의 보존 상태
...
- OS release와 마지막 백업 경로가 포함된 `.knowledge-os/manifest.json` 기반 설치·업그레이드 보고서
- `.knowledge-os-backups/` 아래의 업그레이드 직전 버전별 OS 스냅샷
- 대상 프로젝트에서 실행하는 Portable CLI와 `.knowledge-os/AGENT_BOOTSTRAP.md`
- 운영 이슈 안내가 있는 `.knowledge-os/issues/`와 이후 개선 검토용 사용자 이슈 기록
...
- 사용자 지정 밖 폴더 변경
- `.knowledge-os-backups/`에 `knowledge/` 또는 다른 사용자 자료 포함
- 기존 root `AGENTS.md`의 기존 내용 덮어쓰기·OS 관리 자산 등록
- 기존 root `CLAUDE.md`의 기존 내용 덮어쓰기·OS 관리 자산 등록
- `.knowledge-os/issues/`의 기존 운영 이슈 기록 덮어쓰기·삭제·OS 관리 자산 등록
