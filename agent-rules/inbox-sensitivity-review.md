...
| `agent_mask_context` | `agent_mask_categories`의 `include` 경계에 해당하는 급여·평가·징계 또는 명시적인 불법 콘텐츠 | 해당 텍스트만 마스킹하고 범주·횟수·근거를 Receipt에 기록한 뒤 `completed` |
...
기본 민감도 판단 근거는 개인 이름·이메일이 개인 식별 맥락과 결합된 경우, 고객 문의·불만, 직원·인사 정보,
미공개 가격·매출·전략, 보안 구성·사고 상세다. 계약의 법적 조건·의무·해지·책임·보증·면책, 계약 협상,
...
`agent_mask_categories`에 명시된 `include` 범위의 급여·평가·징계와 명시적인 불법 콘텐츠만 민감도 마스킹 대상이다.
