...
- 탐색은 먼저 `search_knowledge`, `read_bundle`, `prepare_context` 또는 portable CLI의 `search`, `read-bundle`,
  ...
  위한 최후 수단으로 사용할 수 있다. 이 실패·불충분 결과가 지침 부재·모호성 또는 Runtime 결함인지 개별 판단하고,
  후자로 판단한 경우에만 `system-observation` Profile의 `record-system-issue`로 남긴다. fallback 사유와 사용한 범위는 결과에 기록한다.
