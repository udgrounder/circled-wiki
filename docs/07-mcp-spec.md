...
- 목적: 기존 Evidence 한 건을 근거로 신규 Draft Bundle 생성
...
- `verified`는 접근 가능한 Evidence manifest를 요구한다.
...
- 목적: 완료·실패·검토 필요 결과를 Evidence로 수집해 지식 개선 흐름으로 환류
- 입력: task_id, status, summary, feedback, learnings, artifacts, decisions, action_items, open_questions
- 출력: Outcome Evidence ID, 관련 Workflow Bundle, Curator 제안, Learning Signal, 필요 시 개선 Task
- 동일 Task 재호출은 기존 Evidence를 반환해 중복 수집하지 않는다.
