...
- 요약, 태그, 링크 제안
- 신규 `policy`·`decision`·`spec`·`reference` Bundle은 `create_draft_bundle`으로 Draft를 만들고, Guide/Manual·Runbook은 Curation Review·독립 Owner 승인 후에만 Draft로 반영한다. 기존 Bundle은 상태 전환 없이 `apply_bundle_revision`으로 갱신한다.
- OKF 검증 전 결과물 작성
- `Example Organization OKF Profile` 적합성 확인
...
1. Delegator가 요청에 맞는 실행 가능한 Runbook 후보를 찾는다.
2. Hermes가 Runbook을 선택하고 Runtime Task를 준비한다.
3. 필수 입력과 사람 승인을 확인하며 단계를 수행한다.
4. 결과를 완료 기준으로 검증한다.
5. Outcome Inbox Item을 만들고 검사·승인·Evidence 변환을 거쳐 Curator/Reviewer 흐름으로 되돌린다.
