...
1. Agent·사용자·자동화는 실제 문제를 `.circled-wiki/issues/`에 기록한다.
...
6. `verified`에는 `fixed_release`, 재현 명령, 실제 결과, 검증 시각, 검증 artifact가 필요하다.
7. 원인이 같은 이슈는 `duplicate`, 후속 이슈로 교체되면 `superseded`, 재발하면 `reopened` 관계를 기록한다.

### 4.2 원본 프로젝트

1. System Maintainer는 운영 이슈를 읽고 제품 결함, 설치별 설정, 데이터 품질, 운영 절차 문제로 분류한다.
2. 제품 결함만 원본 프로젝트의 개선 작업으로 승격한다. 승격 시 운영 이슈 ID와 재현 조건을 링크한다.
3. 원본에서 최소 하나의 실패 재현 테스트를 만든 뒤 수정한다.
4. 수정은 관련 단위·통합 테스트와 Circled Wiki Validator를 통과해야 한다.
5. 원본의 변경은 검토 후 릴리스 식별자로 묶는다. 운영 이슈를 수정했다는 이유만으로 운영 자료를 Commit하거나 Push하지 않는다.

### 4.3 운영본 배포와 확인

1. 운영자는 먼저 `bootstrap-knowledge-os --target <운영-root>`의 계획을 검토한다.
...
검증 근거를 갱신한다. `[x]`는 명시된 범위만 완료했다는 뜻이며, 상위 항목은 모든 하위 항목과 운영 배포 Gate가
완료되기 전까지 `[ ]`로 유지한다. 체크는 작업 승인이나 자동 배포를 의미하지 않는다.
...
- [ ] **P0-02 Canonical Runtime과 실행 provenance**
  ...
  - [ ] 운영 모델을 순수 설치본 또는 공식 운영 fork 중 하나로 ADR 확정
...
- [ ] **P0-04 Bundle 후보 자동 생성·검토 상태와 Draft → Active Gate**
  ...
  - [ ] Evidence 메타데이터와 후보 이름 생성 경로를 Core API로 통합
    - Evidence ID를 파일 경로·원본 stem·UUID prefix로 역산하거나 glob 검색하지 않고 `find_document_by_id`로 해석
    - Bundle 제목은 확인된 Evidence Record title과 LLM Curator의 정제 결과에서 만들고, title을 읽지 못하면 후보 생성을 차단
    - 파일 slug는 사용자 표시 제목이나 Bundle ID가 아니라 안전한 ASCII path segment로 정규화
    - slug 충돌 시 이미 존재하는 경로를 덮어쓰지 않고 안정적인 Evidence checksum 또는 UUID suffix를 붙여 구분
    - Evidence UUID·Bundle UUID의 앞 8자는 식별, 파일 탐색, 중복 판정, 역참조, slug 충돌 회피에 사용하지 않음
  ...
  - [ ] LLM 출력 계약을 JSON 또는 typed object로 정의하고 자유 형식 Markdown 직접 저장 금지
  ...
  - [ ] LLM이 반환한 Evidence ID가 입력 허용 목록과 정확히 일치하는지 검증하고 임의 URI 생성을 차단
  - [ ] 생성 Body의 PII·자격증명 재검사와 Prompt Injection 영향 검사를 수행
  - [ ] LLM 실패·timeout·schema 오류·근거 부족 시 부분 Bundle을 저장하지 않고 `needs_review` 제안으로 보존
  - [ ] Model provider·model ID·실행 command·timeout·재시도·최대 입력 크기를 typed config로 분리
    - 실제 조직명, Agent 이름, 머신 경로, provider 자격증명은 제품 기본값에 하드코딩하지 않음
    - 모델 미설정 시 자동 후보 생성을 안전하게 비활성화하고 비변경 제안까지만 수행
  - [ ] LLM 호출 전후 checksum, model/version, prompt template version, 결과 schema version을 Curation Receipt로 기록
  - [ ] 동일 Evidence checksum과 Curation Profile 버전의 재실행은 기존 후보를 재사용
  - [ ] 후보 생성 수 상한·비용·토큰 사용량과 Batch별 성공·차단·실패 건수를 보고
  - [ ] `extensions.curation` 후보 메타데이터 schema와 Validator 구현
    - `generated_by`, `generated_at`, `generation_reason`, `evidence_checksum`, `curation_receipt`
    - `recommendation`, `reviewed_by`, `reviewed_at`, `rejection_reason`, `merged_into`
  - [ ] 후보 상태 전이 구현
    - `draft + pending -> draft + needs_changes`
    - `draft + pending -> active + approved`
    - `draft + pending -> archived + rejected|merged`
  - [ ] 현재 전환 API와 우회 경로 재현 테스트 작성
  - [ ] `list-curation-candidates` CLI·MCP 조회 구현
  - [ ] `review|approve|reject|merge-curation-candidate` CLI·operator MCP 구현
  - [ ] 후보 제목·요약·출처·추천 domain/type·기존 Bundle 후보·차단 사유를 사용자에게 표시
  - [ ] 일별 신규·병합·차단·장기 대기 후보 Digest 구현
  - [ ] 동일 Evidence ID와 checksum에 대한 후보 생성 idempotency 구현
  - [ ] PII Scan Receipt, visibility, Evidence checksum, Validator를 통과한 Evidence만 후보화
  - [ ] `propose-pending`의 `recommended_action`과 `blocking_conditions`를 생성 Gate로 강제
  - [ ] `no_bundle` 결과는 Evidence를 처리 완료로 오인하지 않고 사유와 재검토 조건을 기록
  - [ ] 자동화는 기존 Bundle을 직접 갱신하지 않고 `merge` 후보 관계만 기록
...
- [ ] **P0-05 발행 staging·push 권한 경계 복구**
  ...
  - [ ] Commit과 Push의 명시적 API·권한 분리
  - [ ] branch·remote 검증, 동시 실행 lock, Push 재시도 상태 구현
...
- [ ] **P0-06 Inbox 민감성 검토와 PII Scan 자동화 계약**
  ...
  - [ ] 사람 결정 없는 `required` 항목의 승인·ingest·발행 차단 통합 테스트
  - [ ] 실패·재시도·`needs_review`·health receipt 관찰 기능 구현

- [ ] **P0-07 구형 manifest 관리 자산 마이그레이션**
  - [ ] `preserve_existing` 및 구형 marker 자산 인벤토리 산출
  - [ ] 관리 자산 채택 조건과 충돌 proposal 형식 정의
  - [ ] manifest 소유권·checksum 갱신과 rollback 구현
  - [ ] 두 번째 upgrade의 idempotency 및 사용자 수정 보존 테스트
...
- [ ] **P1-03 Workflow 탐색과 Draft 처리 설명 정합화**
  - [ ] 현재 Draft 탐색 동작 재현
  - [ ] Active 실행 조회와 Draft 검토 조회 API 분리
  - [ ] CLI·MCP·문서·테스트 용어 정합화
...
- [ ] **P1-04 운영 파일 배치와 Git hygiene**
  ...
  - [ ] 운영본에서 이미 추적된 생성물의 읽기 전용 목록 산출
  - [ ] 검토된 untrack migration과 rollback 검증

- [ ] **P1-05 이슈 상태·독립 검증자 Gate**
  - [ ] 구현자와 검증자 역할 분리 강제
  - [ ] actor를 상위 인증 주체에 결합하는 계약 구현
  - [ ] fixed release와 검증 artifact 없는 `verified`·`resolved` 전환 차단

- [ ] **P1-06 질의 응답 품질 평가**
  - [ ] 한국어 golden query 세트 작성
  - [ ] 출처·신선도·권한·근거 부족 응답 평가 기준 정의
  - [ ] SNS 마케팅 등 실제 운영 질의 회귀 테스트 자동화

- [ ] **P1-07 Evidence 정제 적체 관찰**
  - [ ] Evidence→Bundle 전환율과 `new` 체류시간 측정
  - [ ] Owner 없는 Draft, 상태별 후보 수, 후보 대기시간과 도메인별 질문 커버리지 측정
  - [ ] 후보 신규 생성·승인·거절·병합 전환율 측정
  - [ ] 임계치 초과 시 review task 또는 운영 Digest 생성

### P2 — 장기 호환성과 표준화

- [ ] **P2-01 Frontmatter 국제화 규칙 정리**
  - [ ] machine-readable field와 사용자 표시 문구 구분 원칙 확정
  - [ ] 기존 혼용 필드 인벤토리와 migration guide 작성

- [ ] **P2-02 Circled Wiki 명칭 전환**
  - [ ] 사용자 노출 명칭의 legacy `Knowledge OS` 인벤토리 산출
  - [ ] CLI·Python package 호환 alias와 deprecation 기간 결정
  - [ ] 설치·upgrade·운영 문서의 사용자 표기 통일
...
2. 전체 테스트와 `PYTHONPATH=src python3 -m knowledge_os.cli validate`가 통과한다.
...
공식 제품명과 사용자 표시 이름은 `Circled Wiki`로 통일한다. 기존 `knowledge-os` CLI 명령과 Python `knowledge_os`
