...
- [x] **P2-02 Circled Wiki 명칭 전환**
  - [x] 사용자 노출 명칭의 legacy `Knowledge OS` 인벤토리 산출
  ...
  - [x] CLI·Python package 호환 alias와 deprecation 기간 결정
    - [x] `circled-wiki` CLI alias 추가, legacy `knowledge-os` CLI와 `knowledge_os` package 유지
...
- [x] **2026-07-22 Obsidian Evidence 링크 분리** — `evidence://...`는 영구 Evidence ID로 유지하고,
  Bundle `evidence_links`와 preserved source `uri`는 `knowledge/` root 기준 `evidence/.../*.md` 경로로 생성;
