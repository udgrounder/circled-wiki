...
Collection Agent <── release·위치 기준·수집 안내 ── Wiki Agent / Runtime
```
...
수집 Agent는 설치 root에서 다음을 실행해 현재 수집 안내를 받는다.

```sh
circled-wiki get-collection-handoff
```

수집 Agent는 시작 시, Wiki release 변경 시 또는 오류 뒤에 원문·credential·활성 PII 없이 계약을 요청한다. 응답의
`guidance_document`가 가리키는 `.circled-wiki/contracts/COLLECTION_HANDOFF.md`와 `guidance_markdown`은 수집 품질을 높이기 위한 행동 지침이며, 수집 성공의 필수 조건은 아니다.
수집 Agent는 이 위치에 **새 Markdown 파일만** 만든다.
...
`recommended_frontmatter`는 다음 정보를 함께 수집하도록 안내한다. 이 중 누락된 값은 추정하지 않는다.
...
`jsonschema` 등 Wiki Runtime 오류로 계약 조회 자체가 실패해도 수집을 중단하지 않는다. 기존 Inbox Rule의 provider Inbox에 새 raw 파일을
...
기존 Inbox Rule의 provider 폴더에 원본 파일을 새로 보존하고 최소한 provider·수집 시각·출처를 안전한 작업 로그에 남긴다.
...
현재 release는 위치 기준·권장 Frontmatter·release-aware 수집 안내를 제공한다. 수집 Agent는 기존 Inbox Rule의 provider 폴더에 새 파일을 생성하고,
