...
| 분류 완료 Inbox 항목을 안전한 선행 단계부터 Evidence까지 자동 복구 | `agent-rules/contracts/inbox.yaml`을 적용하고 `inbox-inspection.md` → `evidence-ingest.md`를 순서대로 적용. 업무성 분류·격리·처분 판단은 자동 수행하지 않음 |
| Evidence Curation Queue를 결과 상태까지 자동 복구 | `agent-rules/contracts/curation.yaml`을 적용하고 `knowledge-curation.md` Gate를 적용. 검증된 결과 상태 공유에는 `publication.md`로 전환. 의미 변경 승인·revision 적용은 제외 |
