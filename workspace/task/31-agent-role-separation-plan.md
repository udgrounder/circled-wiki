...
지정했을 때만 `workspace/issue/inbox/`로 이동한다. 이동 대상은 Git에 추적·커밋된 파일이어야 하며, 이동에 실패하면
원래 위치에 남는다. 사용자와 검토하고 필요한 처리를 완료한 Issue는 `workspace/issue/archived/`로 이동한다.

```mermaid
flowchart LR
  O["운영 문제 관찰"] --> I["운영 Issue 기록"]
  I -->|"사용자 수집 요청"| C["workspace/issue/inbox 반입"]
  C --> U["사용자 검토"]
  U --> T["원본 프로젝트 Triage"]
  ...
  L --> A["workspace/issue/archived 이동"]
  S --> A
```
...
3. 사용자가 지정한 운영 프로젝트의 Issue만 `workspace/issue/inbox/`에 수집하고 사용자 검토 Gate를 적용한다.
4. 운영 Issue와 제품 변경 사이에 명시적인 Triage·Release·Deployment handoff를 둔다.
5. 검토·처리가 끝난 Workspace Issue를 버전 정보와 함께 `workspace/issue/archived/`로 이동한다.
...
| Workspace Issue Item | 사용자가 지정한 Git 추적 운영 Issue를 `workspace/issue/inbox/`로 이동한 현재 작업 항목 |
...
| 원본 `workspace/issue/inbox/` | 명시적 사용자 요청으로 이동된 Issue 관리 | 접근 금지 | 접근 금지 |
| 원본 `workspace/issue/archived/` | 검토·처리 Gate 후 Issue 버전 보존 | 접근 금지 | 접근 금지 |
...
2. 대상 파일을 `workspace/issue/inbox/<project-ref>/<source-issue-id>.md`로 `mv`한다.
...
모든 `pending_review` Issue는 사용자 검토 전에 `workspace/issue/archived/`의 기존 해결 이력을 조회한다. 정확히 같은
...
다음 조건을 모두 충족한 Workspace Issue Item만 `workspace/issue/archived/`로 이동한다.
...
- 지정 Issue를 `workspace/issue/inbox/`로 이동
...
- `workspace/issue/inbox/<project-ref>/<source-issue-id>.md`
...
1. `workspace/issue/README.md`, `workspace/issue/inbox/`, `workspace/issue/archived/` 구조와 Git 추적 정책을 정의한다.
