...
TOOLS = [
    ...
    {"name": "capture_file", "description": "Land a PDF, Word, HTML, or other source file as a pending Inbox item with an original-preserving envelope; it does not ingest or curate it. Capture always starts sensitivity_review as required.", "inputSchema": {"type": "object", "required": ["payload_base64", "original_filename", "provider", "title", "why_collected", "intended_use", "idempotency_key"], "properties": {"payload_base64": {"type": "string", "minLength": 1}, "original_filename": {"type": "string"}, "provider": {"type": "string"}, "title": {"type": "string"}, "why_collected": {"type": "string"}, "intended_use": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "idempotency_key": {"type": "string"}, "source_url": {"type": "string"}, "source_locator": {"type": "string"}, "captured_from": {"type": "string", "enum": ["api", "webhook", "manual", "upload", "sync"], "default": "upload"}}}},
