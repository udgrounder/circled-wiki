...
TOOLS = [
    ...
    {"name": "review_inbox_sensitivity", "description": "Record an Agent sensitivity-review decision with the selected policy, four performed checks, observed categories, and rationale. Request the user only when no applicable basis exists.", "inputSchema": {"type": "object", "required": ["intake_id", "actor", "decision", "policy_ref", "checks", "matched_categories", "rationale"], "properties": {"intake_id": {"type": "string"}, "actor": {"type": "string"}, "decision": {"type": "string", "enum": ["completed", "not_applicable"]}, "policy_ref": {"type": "string", "const": "inbox-sensitivity/v1"}, "checks": {"type": "array", "minItems": 4, "maxItems": 4, "items": {"type": "string"}}, "matched_categories": {"type": "array", "items": {"type": "string"}}, "rationale": {"type": "string", "minLength": 1}}}},
    ...
    if method == "initialize":
        ...
        try:
            ...
            if name == "search_knowledge": content = service.search_knowledge(arguments["query"], arguments.get("filters"))
                ...
                policy_ref=arguments["policy_ref"], checks=arguments["checks"],
                matched_categories=arguments["matched_categories"], rationale=arguments["rationale"],
            )
