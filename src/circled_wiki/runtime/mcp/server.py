...
TOOLS = [
    {"name": "search_knowledge", "description": "Search knowledge Bundles and Evidence Records.", "inputSchema": {"type": "object", "required": ["query"], "properties": {"query": {"type": "string"}, "filters": {"type": "object"}}}},
    ...
    {"name": "capture_conversation", "description": "Create and validate a pending Inbox conversation from collected content. A collection Agent may call this directly, or hand the same inputs to the Wiki Agent for proxy execution.", "inputSchema": {"type": "object", "required": ["content", "provider", "title", "why_collected", "intended_use", "idempotency_key"], "properties": {"content": {"type": "string", "minLength": 1}, "provider": {"type": "string"}, "title": {"type": "string"}, "why_collected": {"type": "string"}, "intended_use": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "idempotency_key": {"type": "string"}, "thread_ref": {"type": "string"}, "turn_from": {"type": "integer", "minimum": 0}, "turn_to": {"type": "integer", "minimum": 0}, "artifacts": {"type": "array", "items": {"type": "object"}}}}},
    {"name": "capture_document", "description": "Create and validate a pending Inbox document from collected source content and provenance. A collection Agent may call this directly, or hand the same inputs to the Wiki Agent for proxy execution.", "inputSchema": {"type": "object", "required": ["content", "provider", "title", "why_collected", "intended_use", "idempotency_key"], "properties": {"content": {"type": "string", "minLength": 1}, "provider": {"type": "string"}, "title": {"type": "string"}, "why_collected": {"type": "string"}, "intended_use": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "idempotency_key": {"type": "string"}, "source_url": {"type": "string"}, "source_locator": {"type": "string"}, "captured_from": {"type": "string", "enum": ["api", "webhook", "manual", "upload", "sync"], "default": "sync"}}}},
    {"name": "capture_file", "description": "Create and validate a versioned pending Inbox file envelope while preserving the original payload. A collection Agent may call this directly, or hand the same v1 input to the Wiki Agent for proxy execution.", "inputSchema": {"type": "object", "required": ["payload_base64", "original_filename", "provider", "title", "why_collected", "intended_use", "idempotency_key"], "properties": {"contract_version": {"type": "string", "enum": ["v1"], "default": "v1"}, "payload_base64": {"type": "string", "minLength": 1}, "original_filename": {"type": "string"}, "provider": {"type": "string"}, "title": {"type": "string"}, "why_collected": {"type": "string"}, "intended_use": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "idempotency_key": {"type": "string"}, "source_url": {"type": "string"}, "source_locator": {"type": "string"}, "captured_from": {"type": "string", "enum": ["api", "webhook", "manual", "upload", "sync"], "default": "upload"}}}},
    ...
    {"name": "record_inbox_pii_scan", "description": "Attach a low-level external scanner result as a compatibility input. It never resolves the integrated Data Protection Review or creates the final data_protection_receipt; call review_data_protection for the single acceptance Receipt.", "inputSchema": {"type": "object", "required": ["intake_id", "scanner", "scanner_version", "result", "reviewed_by", "receipt"], "properties": {"intake_id": {"type": "string"}, "scanner": {"type": "string"}, "scanner_version": {"type": "string"}, "result": {"type": "string", "enum": ["passed", "masked", "needs_review"]}, "reviewed_by": {"type": "string"}, "receipt": {"type": "string"}, "scanned_at": {"type": "string", "format": "date-time"}}}},
    {"name": "list_inbox_review_queue", "description": "List exceptional Inbox contract tasks that are awaiting a user-only decision. Agent reprocessing work is not included in this view.", "inputSchema": {"type": "object", "properties": {}}},
    ...
    {"name": "review_data_protection", "description": "Run the integrated PII Scan and Agent sensitivity review. Mask Agent-identified restricted text findings, retain permitted business contact context, and record a safe receipt without retaining finding values.", "inputSchema": {"type": "object", "required": ["intake_id", "actor", "context", "checks", "rationale"], "properties": {"intake_id": {"type": "string"}, "actor": {"type": "string"}, "context": {"type": "string"}, "checks": {"type": "array", "minItems": 4, "maxItems": 4, "items": {"type": "string"}}, "rationale": {"type": "string", "minLength": 1}, "findings": {"type": "array", "items": {"type": "object", "required": ["category", "value"], "properties": {"category": {"type": "string"}, "value": {"type": "string", "minLength": 1}}}}}}},
    {"name": "request_inbox_sensitivity_decision", "description": "Move an unresolved sensitivity inspection to awaiting_user without copying the source. Record the exact question, blocked step, missing procedure, safe next action, facts, and hypotheses.", "inputSchema": {"type": "object", "required": ["intake_id", "actor", "question", "missing_procedure", "safe_next_action", "facts", "hypotheses"], "properties": {"intake_id": {"type": "string"}, "actor": {"type": "string"}, "question": {"type": "string", "minLength": 1}, "missing_procedure": {"type": "string", "minLength": 1}, "safe_next_action": {"type": "string", "minLength": 1}, "facts": {"type": "array", "items": {"type": "string"}}, "hypotheses": {"type": "array", "items": {"type": "string"}}}}},
    ...
    {"name": "reconcile_inbox", "description": "Use the installed Inbox contract to advance ready pending items through acceptance and accepted items through Evidence ingestion. It never infers sensitivity or PII review decisions.", "inputSchema": {"type": "object", "required": ["actor"], "properties": {"actor": {"type": "string"}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}}}},
    {"name": "reconcile_curation", "description": "Use the installed Curation contract to run configured analysis, durable no_bundle decisions, and Review Queue creation results. It never approves meaning changes or applies revisions.", "inputSchema": {"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}}}},
    ...
    {"name": "decide_curation_review", "description": "Record a reviewer decision. A successful new-Draft approval records the decision in the Bundle; consumed review cards move to the hidden archive, while Evidence remains unchanged.", "inputSchema": {"type": "object", "required": ["review_id", "action", "actor"], "properties": {"review_id": {"type": "string"}, "action": {"type": "string", "enum": ["approve", "no_bundle", "needs_changes", "needs_review"]}, "actor": {"type": "string"}, "note": {"type": "string"}}}},
    {"name": "apply_approved_curation_update", "description": "Apply exactly one approved, checksum- and revision-bound update review to its existing Bundle, then archive the consumed Review card.", "inputSchema": {"type": "object", "required": ["review_id", "actor"], "properties": {"review_id": {"type": "string"}, "actor": {"type": "string"}}}},
    {"name": "create_curation_review", "description": "Create a checksum-bound manual or runbook review card from an explicit validated curation proposal.", "inputSchema": {"type": "object", "required": ["evidence_id", "output", "generated_by", "curation_receipt"], "properties": {"evidence_id": {"type": "string"}, "output": {"type": "object"}, "generated_by": {"type": "string"}, "curation_receipt": {"type": "string"}}}},
    ...
    if method == "initialize":
        ...
        try:
            ...
            if name == "search_knowledge": content = service.search_knowledge(arguments["query"], arguments.get("filters"))
                ...
                reviewed_by=arguments["reviewed_by"], receipt=arguments["receipt"],
                scanned_at=arguments.get("scanned_at"),
            )
            elif name == "review_data_protection": content = service.review_data_protection(
                arguments["intake_id"], arguments["actor"], context=arguments["context"],
                checks=arguments["checks"], rationale=arguments["rationale"],
                findings=arguments.get("findings"),
            )
            elif name == "request_inbox_sensitivity_decision": content = service.request_inbox_sensitivity_decision(
                arguments["intake_id"], arguments["actor"], question=arguments["question"],
                missing_procedure=arguments["missing_procedure"], safe_next_action=arguments["safe_next_action"],
                facts=arguments["facts"], hypotheses=arguments["hypotheses"],
            )
            elif name == "accept_inbox": content = service.accept_inbox(arguments["intake_id"], arguments["actor"])
            elif name == "accept_ready_inbox": content = service.accept_ready_inbox(arguments["actor"], arguments.get("limit", 100))
            elif name == "reconcile_inbox": content = service.reconcile_inbox(arguments["actor"], arguments.get("limit", 100))
            elif name == "reconcile_curation": content = service.reconcile_curation(arguments.get("limit", 100))
            ...
            elif name == "apply_approved_curation_update": content = service.apply_approved_curation_update(
                arguments["review_id"], actor=arguments["actor"],
            )
            elif name == "create_curation_review": content = service.create_curation_review(arguments["evidence_id"], arguments["output"], generated_by=arguments["generated_by"], curation_receipt=arguments["curation_receipt"])
                ...
                automated=bool(arguments.get("automated", False)),
            )
