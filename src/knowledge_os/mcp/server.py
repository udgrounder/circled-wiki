...
TOOLS = [
    ...
    {"name": "record_evidence_pii_scan", "description": "Record a scanner or identified reviewer's PII scan receipt bound to the current Evidence checksum. This records a supplied result; it does not run a scanner.", "inputSchema": {"type": "object", "required": ["evidence_id", "scanner", "scanner_version", "result", "reviewed_by", "receipt"], "properties": {"evidence_id": {"type": "string"}, "scanner": {"type": "string"}, "scanner_version": {"type": "string"}, "result": {"type": "string", "enum": ["passed", "masked", "needs_review"]}, "reviewed_by": {"type": "string"}, "receipt": {"type": "string"}, "scanned_at": {"type": "string", "format": "date-time"}}}},
    ...
    {"name": "decide_curation_review", "description": "Record a reviewer decision. Only approve creates a new Draft; no_bundle preserves Evidence and prevents repeat review for the same checksum.", "inputSchema": {"type": "object", "required": ["review_id", "action", "actor"], "properties": {"review_id": {"type": "string"}, "action": {"type": "string", "enum": ["approve", "no_bundle", "needs_changes", "needs_review"]}, "actor": {"type": "string"}, "note": {"type": "string"}}}},
...
def handle_request(
    ...
    if method == "initialize":
        ...
        try:
            ...
            elif name == "record_evidence_pii_scan": content = service.record_evidence_pii_scan(
                arguments["evidence_id"], scanner=arguments["scanner"],
