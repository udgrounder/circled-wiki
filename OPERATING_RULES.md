...
- **RB-ROUTE-008** 지식 변경은 `capture_* -> inspect_inbox -> review-data-protection -> accept_inbox -> ingest_accepted -> propose_update -> create_draft_bundle | apply_bundle_revision` 흐름을 사용한다. `ingest_evidence`는 이 Gate를 통과한 후보를 변환하는 Runtime 내부 구현이며, CLI·MCP·Service의 직접 실행 경로가 아니다.
