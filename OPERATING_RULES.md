...
- **RB-SEC-001** 지원되는 PII 범주의 자동 처리는 `.circled-wiki/data-protection.yaml`의 `pii_scan.hard_mask_categories` 토글을 따른다. `true`인 주민등록번호·계좌번호·카드번호·자격증명(API key·token·password·private key 등)과 설정된 `agent_mask_categories`에 따라 마스킹한 값은 Bundle, Evidence, Task, Log, Prompt에 평문으로 기록하지 않으며, `false`인 범주는 이 자동 하드 PII 경로에서 처리하지 않는다. `010`·`+82 10` 휴대전화와 이메일은 기능적으로 지원하지만 기본 템플릿에서 `false`다. 이미 마스킹된 값은 토글을 끄더라도 복원하지 않는다. `non_sensitive_categories`로 분류된 구성원·협력업체 업무 연락처는 내부 Receipt·Task·Log·Prompt·Evidence·Bundle에 업무상 필요한 경우 원문을 보존할 수 있으며, 이를 내부 운영 기록의 금지 항목으로 간주하지 않는다. 계약·법률 자문·분쟁·소송·규제 대응과 그 결정은 법무 업무라는 이유만으로 이 정책에서 제한하지 않으며, 명시적인 불법성이 확인되지 않은 법률·계약 내용도 자동 제한하지 않는다. 외부 전송·게시·발행에서는 별도 visibility·Publication Gate와 수신 목적에 따라 제한 또는 마스킹을 적용한다. 모든 Inbox 수집 주체는 공통 Capture 단계의 민감정보 사전 점검을 거치며, 통합 Data Protection Review가 활성 하드 범주 재확인·민감도 판단·최종 checksum을 하나의 Receipt에 기록한다. 정책에 없는 업무 맥락 정보는 자동 하드 PII 경로에서 처리하지 않으며, 실제 마스킹 대상의 처리 방식이 불명확할 때만 별도 사람 검토를 요청한다.
...
- **RB-PUB-013** Evidence 생성, Review Queue 생성·갱신, Curation Review·Draft 생성, Curation Queue의 재시도·차단 기록과 Bundle 발행은 유효한 상태 전이다. 같은 처리 주체가 연속 수행하는 상태 전이는 로컬 작업 단위로 묶을 수 있다. 처리 주체 변경 또는 최종 종료 전이만 PII·보안 Gate, Validator, 허용 경로·clean staging 검사를 통과한 뒤 configured publication policy에 따라 Commit·Push하여 다음 실행 주체가 볼 수 있게 한다. **처리 주체·작업 상태 전이 Receipt**에는 이전 결과·대상 checksum·다음 주체·다음 `next_action`을 기록한다. Push Receipt가 없으면 `publication_pending`으로 유지하며 다음 업무 단계로 진행하지 않는다. 의미적 최종 승인 여부와 Commit·Push 여부를 혼동하지 않는다. 보안 Gate의 `needs_review`는 실패가 아니라 `awaiting_user`이다.
