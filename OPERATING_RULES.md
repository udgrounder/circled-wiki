...
- **RB-PUB-013** Evidence 생성, Review Queue 생성·갱신, Curation Review·Draft 생성, Curation Queue의 재시도·차단 기록과 Bundle 발행은 유효한 상태 전이다. 같은 처리 주체가 연속 수행하는 상태 전이는 로컬 작업 단위로 묶을 수 있다. 처리 주체 변경 또는 최종 종료 전이만 PII·보안 Gate, Validator, 허용 경로·clean staging 검사를 통과한 뒤 configured publication policy에 따라 Commit·Push하여 다음 실행 주체가 볼 수 있게 한다. Curation Queue 완료가 포함되면 `workspace/task/curation_reconciliation/<uuid>.md` 삭제와 새 `workspace/task/.archive/curation_reconciliation/<uuid>.md` 추가를 `verify-curation-commit`으로 같은 staged 전환 쌍인지 확인한다. 한쪽만 staged 된 경우 Commit·manifest·Receipt를 만들지 않고 정확한 두 경로를 staging해 재시도한다. **처리 주체·작업 상태 전이 Receipt**에는 이전 결과·대상 checksum·다음 주체·다음 `next_action`을 기록한다. Push Receipt가 없으면 `publication_pending`으로 유지하며 다음 업무 단계로 진행하지 않는다. 의미적 최종 승인 여부와 Commit·Push 여부를 혼동하지 않는다. 보안 Gate의 `needs_review`는 실패가 아니라 `awaiting_user`이다.
