...
- **RB-KNW-024** `knowledge/` 루트의 운영·관리 안내는 `README.md` 하나로 관리한다. README에는 사람과 Agent가 Vault를 운영하는 데 필요한 최상위 폴더 구조·각 폴더의 역할·운영 흐름·보관·검토 기준만 기록하며, 개별 Evidence·Bundle·Inbox 문서의 목록·요약·세부 내용은 기록하지 않는다. Bootstrap은 빈 Knowledge Vault의 최초 설치 때에만 이 README를 생성한다. 운영·관리에 필요한 정보가 새로 생기거나 변경되면, 해당 변경에서 README의 관련 설명도 함께 갱신한다. `index.md` 등 안내·탐색 문서를 이동·삭제하기 전에는 그 문서에서 계속 필요한 운영·관리 정보를 README에 먼저 통합하고, 구조 변경 뒤 README가 실제 폴더 구조와 일치하는지 확인한다. 개별 Evidence·Bundle·Inbox 항목의 추가·갱신만으로 README를 변경하지 않는다. 자동화는 하위 `index.md` 또는 `README.md`를 생성·갱신·삭제하지 않는다. `knowledge/inbox/`와 그 provider 하위 폴더는 처리 대기 입력 영역이므로 Bootstrap·수집·검사·변환·정제 작업이 안내 문서를 생성하거나 변경하지 않는다.
