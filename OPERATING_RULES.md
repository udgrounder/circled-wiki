...
- **RB-ROUTE-017** 작업은 동일 처리 주체가 연속으로 수행하는 단계들을 하나의 로컬 작업 단위로 묶는다. `agent -> user`, `user -> agent`, 또는 최종 종료 상태로 전환할 때만 handoff 또는 종료 Receipt에 현재 결과·대상 checksum·다음 `next_action`·다음 처리 주체를 기록하고 Commit·Push한다. Push Receipt가 없으면 해당 handoff·종료 전이는 `publication_pending`이며, 다음 업무 단계로 진행하지 않고 Push를 재시도한다.
