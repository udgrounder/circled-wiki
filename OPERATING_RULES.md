...
| Inbox Sensitive Data Review | PII 유형과 별개로, Capture가 기본값 `required`로 시작한 Inbox Item의 수집·변환 가능 여부를 Agent가 `inbox-sensitivity/v1` 정책·검사 Receipt의 근거에 따라 `sensitivity_review`로 판단하는 단계. `completed` 또는 `not_applicable`은 식별된 실행 주체의 명시적 결정이며, 적용할 정책·검사 근거가 부족할 때만 `required`로 유지해 사용자에게 문의한다. |
| Evidence PII Scan | 전화번호를 포함한 정책 대상 PII를 Evidence 후보 원문과 Git 추적 예정 텍스트에서 실제 검사하고 필요한 마스킹을 적용한 뒤, 후보 checksum에 결합된 `pii_scan` 영수증과 `pii_scanned` 상태를 Evidence 최초 생성 입력으로 확정하는 단계. Inbox Sensitive Data Review 완료만으로 대체할 수 없다. |
...
- **RB-ROUTE-008** 지식 변경은 `capture_* -> inspect_inbox -> review_inbox_sensitivity -> accept_inbox -> ingest_accepted -> propose_update -> create_draft_bundle | apply_bundle_revision` 흐름을 사용한다. `ingest_evidence`는 이 Gate를 통과한 후보를 변환하는 Runtime 내부 구현이며, CLI·MCP·Service의 직접 실행 경로가 아니다.
...
- **RB-ROUTE-011** Inbox 수집은 전체 저장소 테스트나 Bundle 정제를 실행하지 않는다. `inspect_inbox`는 읽기 전용, `review_inbox_sensitivity`는 Agent의 근거 있는 민감성 결정, `accept_inbox`는 검사 Gate, `ingest_accepted`는 Evidence 직전 PII Scan·Receipt 확정과 Evidence 변환만 담당하며 `propose_pending`이 정제를 별도로 수행한다. 세부 전이·차단 사유·재처리는 등록된 Inbox 계약과 선택된 Profile을 따른다.
...
- **RB-ROUTE-016** Curation 계약은 configured Curator의 분석 결과를 `no_bundle_recorded`, `review_handoff`, 자동 Gate를 통과한 `published`, Gate 실패로 남은 `draft_created`, 재시도 가능한 `queued` 중 하나로 기록한다. 각 outcome은 Queue 완료 또는 유지 여부를 함께 검증한다. `runbook`·`manual`을 제외한 기존 Bundle의 갱신은 configured Curator의 후보가 제안 대상과 일치하고 확정된 Evidence·보안·Validator·대상 revision Gate와 자동 갱신 Receipt를 통과했을 때 `published` outcome으로 기록한다. Curation은 Evidence 생성 시 확정한 민감도·PII 판단을 재실행하지 않는다. `runbook`·`manual`의 생성·갱신·승격은 기존 Review·Owner·checksum·revision Gate를 별도 호출로 통과해야 한다. 계약 재조정이 이를 대신하지 않는다.
...
- **RB-EVD-020** PII Scan은 Inbox에서 Evidence로 넘길 후보의 PII 처리 기준을 확정하는 단일 단계다. Capture와 PII Scan은 같은 정책 모듈을 사용하되, PII Scan은 전화번호를 포함한 정책 대상 PII를 실제 Inbox 후보에서 다시 검사해 필요한 마스킹·최종 후보 checksum 계산·Scanner·버전·시각·결과·검토자·Receipt 기록을 확정한다. PII 유형별 예외로 `sensitivity_review` 또는 acceptance를 자동 해소하지 않는다. `passed`와 `masked`는 그 최종 후보 checksum에 결합된 Receipt를 Inbox에 기록하고, 같은 후보만 Evidence 최초 생성 입력으로 사용한다. 동일 후보 checksum의 유효한 Receipt는 생성 주체와 관계없이 이후 작업에서 그대로 재사용하며, 상태 자체를 재검토 차단 사유로 쓰지 않는다. 후보 checksum이 바뀌거나 사용자·Agent가 재검토를 요청하면 기존 Receipt의 효력이 끝나고 작업은 PII Scan 단계로 되돌아간다. 그 단계에서 새 후보 기준으로 자동 Scan 또는 사용자 검토를 결정한다. Inbox Review Queue 카드의 식별·연관·상태 재개는 Intake ID의 UUID만 사용하며, checksum은 Queue 연관이 아니라 후보 파일 변경 검출과 Receipt 검증에만 사용한다. `needs_review`는 오류가 아닌 PII Scan Receipt의 판정이며, 안전 처리가 사용자만 수행할 수 있을 때 작업 Queue를 `awaiting_user`로 전환한다. 이 경우 Evidence를 생성하지 않고 Inbox와 Review Queue만 갱신하며, Queue 상태·Receipt·다음 검토 행동은 검증 후 공유 가능한 저장소 상태로 Commit·Push한다. 이 단계는 Inbox Sensitive Data Review, 접근 권한 또는 Publication Security Review를 대체하지 않으며, 생성된 Evidence의 Scan 상태나 Receipt는 변경하지 않는다.
...
accepted Evidence
  -> Curator
  -> runbook/manual: Curation Review 카드 -> 별도 검증 시도 -> Draft 또는 revision 제안
  -> 그 밖의 유형: 자동 Draft/갱신 -> 자동 Gate
...
- **RB-CUR-001** `runbook`과 `manual` 신규 Bundle 정제 제안은 Evidence checksum, 생성 actor와 제안 내용을 가진 `curation_review` 카드로 먼저 기록한다. `policy`, `guide`, `decision`, `spec`, `reference`, `report` Draft는 확정된 Evidence를 입력으로 직접 생성할 수 있다. Evidence 생성 시 확정된 PII Scan은 Curation에서 재실행하지 않는다.
...
- **RB-CUR-004** `runbook`·`manual` Review가 필요한 유형에서 카드가 없으면 Draft 생성·revision·Promotion을 중단한다. 운영 Agent의 자동 Curation은 이 두 유형 외에는 Review 카드를 생성할 수 없다. `policy`, `guide`, `decision`, `spec`, `reference`, `report`의 Review 카드는 사용자가 명시적으로 요청했고 Runtime이 그 요청 식별자(`user_review_request`)를 함께 기록한 경우에만 생성할 수 있다. Curation Queue 재처리는 동일 Evidence의 미완료 Review 카드를 먼저 찾아 재사용하고, 카드가 없을 때만 새 UUID 카드를 생성한다. Review 카드가 생성되면 Curator는 현재 요청 대화에서 승인 선택지를 묻지 않고 `knowledge/curation-reviews/`에 Review 카드 생성 결과를 기록하고 종료한다. `update_existing`는 대상으로 읽은 본문 checksum(`base_body_checksum`)과 변경 방식(`update_mode`)을 반드시 기록한다. `append`는 Runtime이 기존 본문을 보존한 채 delta를 추가하며 자동 갱신의 유일한 본문 변경 방식이다. 전체 본문 교체(`replace_full`)는 교체 사유가 있는 사용자·검토 경로에서만 허용한다. `runbook`·`manual` 승인 또는 `update_existing` 적용 직전에는 현재 Evidence checksum·대상 Bundle revision·기준 본문 checksum을 다시 확인하며, 달라지면 stale 처리한다. 그 밖의 기존 Bundle 갱신은 후보 Bundle의 `type`·도메인·태그·Evidence 참조 Frontmatter가 제안과 일치하고, 현재 Evidence의 `source_ref.snapshot_at` 또는 `captured_at`이 대상 Bundle의 최신 Evidence보다 같거나 최신이며, Security Receipt·대상 revision·전체 Validator·자동 갱신 Receipt를 통과할 때 자동 적용한다. 일치 후보가 없으면 신규 Bundle을 생성한다. 이 자동 Gate가 실패하면 Review로 바꾸지 않고 Curation Queue에 retryable blocker를 남긴다. 여기서 Evidence checksum은 이미 확정된 Evidence의 동일성 확인이며 PII Scan 재검토가 아니다. stale 카드는 숨김 archive로 보존하고 해당 Evidence를 Curation Queue에 다시 등록해 현재 상태 기준 제안을 새로 만든다.
- **RB-CUR-005** `draft -> active` 상태 전환은 전용 Promotion Gate만 수행한다. 일반 Bundle 생성·revision API와 직접 Frontmatter 변경은 active 전환 수단이 아니다.
- **RB-CUR-006** 모든 active Bundle에는 승격 actor·시각, Security Receipt, 현재 Evidence checksum과 Evidence 생성 시 확정된 PII Scan Receipt를 provenance로 남긴다. `runbook`과 `manual`에는 생성 전 Review ID와 RB-CUR-003의 별도 검증 기록이 필수다. 최초 생성 Review와 이후 보완 Review는 `extensions.curation.review_receipts`에 Review ID·검증 시도 ID·검증 주체·시각·Evidence checksum·적용 revision·결정 메모로 누적 보존하며 최초 생성 Review를 덮어쓰지 않는다. `policy`, `guide`, `decision`, `spec`, `reference`, `report`는 확정된 Evidence·참조 무결성·전체 Validator Gate를 통과하면 자동으로 `active`로 승격할 수 있으며, provenance에 자동 승격임을 기록한다.
