...
- **RB-ROUTE-011** Inbox 수집은 전체 저장소 테스트나 Bundle 정제를 실행하지 않는다. `inspect_inbox`는 읽기 전용, `review_inbox_sensitivity`는 식별된 사람의 민감성 결정, `accept_inbox`는 검사 Gate, `ingest_accepted`는 Evidence 직전 후보의 PII Scan·Receipt 확정과 Evidence 변환만 담당하며 `propose_pending`이 정제를 별도로 수행한다. `reconcile_inbox`는 `agent-rules/contracts/index.yaml`에 등록된 Inbox 계약에 따라 이 순서의 검사·수용·PII Scan·Evidence 변환 Gate와 상태 기록을 유지하는 범위에서만 안전한 선행 단계를 자동 재수행할 수 있다. 자동 PII Scan은 실제 후보를 검사해 `passed` 또는 `masked` Receipt를 확정할 수 있다. `needs_review`는 `awaiting_user` Queue 상태로 남기며, 민감성 판단, `needs_review` 뒤 안전 처리, 승인 판단을 건너뛰거나 추정하지 않는다.
...
- **RB-ROUTE-015** `agent-rules/contracts/index.yaml`에 등록된 영역별 단계 계약은 현재 Frontmatter와 Queue의 상태를 기준으로 안전한 선행 단계를 재수행할 수 있다. 계약은 RB-EVD-020의 자동 PII Scan·Receipt 확정을 대체하지 않으며, 승인·민감성 판단·`needs_review` 뒤 안전 처리 결정을 추정하거나 대체하지 않는다. 해당 사람 결정이 없으면 기존 Queue와 상태를 유지하고 안전한 다음 행동만 반환한다.
...
- **RB-ROUTE-017** 작업은 동일 처리 주체가 연속으로 수행하는 단계들을 하나의 로컬 작업 단위로 묶는다. 아직 Commit하지 않은 같은 주체의 로컬 작업은 다음 작업과 함께 한 번에 공유할 수 있다. `agent -> user`, `user -> agent`, 또는 최종 종료 상태로 전환할 때만 handoff 또는 종료 Receipt에 현재 결과·대상 checksum·다음 `next_action`·다음 처리 주체를 기록하고 Commit·Push한다. 이 Commit의 Push가 실패하면 해당 handoff·종료 전이는 `publication_pending`이다. 다음 Push는 `resume-pending-push`로 현재 HEAD와 원격에 없는 모든 선행 Commit을 함께 전송하며, 성공 전에는 handoff를 다음 처리 주체가 받은 것으로 보지 않는다.
...
- **RB-PUB-013** Evidence 생성, Review Queue 생성·갱신, Curation Review·Draft 생성, Curation Queue의 재시도·차단 기록과 Bundle 발행은 유효한 상태 전이다. 같은 처리 주체가 연속 수행하는 상태 전이는 로컬 작업 단위로 묶을 수 있다. 처리 주체 변경 또는 최종 종료 전이만 PII·보안 Gate, Validator, 허용 경로·clean staging 검사를 통과한 뒤 configured publication policy에 따라 Commit·Push하여 다음 실행 주체가 볼 수 있게 한다. handoff Receipt에는 이전 결과·대상 checksum·다음 주체·다음 `next_action`을 기록한다. Push Receipt가 없으면 `publication_pending`으로 유지하며 다음 업무 단계로 진행하지 않는다. 의미적 최종 승인 여부와 Commit·Push 여부를 혼동하지 않는다. 보안 Gate의 `needs_review`는 실패가 아니라 `awaiting_user`이다.
