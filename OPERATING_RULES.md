...
| Data Protection Review | Capture가 기본값 `required`로 시작한 Inbox Item에 대해 `data-protection.yaml`의 활성 하드 PII를 마스킹하고, 같은 범주를 다시 확인해 누락을 보완하며, 명시된 정책 판단 후보와 Wiki별 민감도 근거에 따라 Agent의 선택적 마스킹을 같은 단계에서 수행하고, 최종 후보 checksum에 결합된 단일 `data_protection_receipt`를 확정하는 단계. `passed` 또는 `masked`만 acceptance와 Evidence 변환을 허용한다. |
| Evidence Transition Verification | Evidence 생성 시 Manifest Validator·checksum을 한 번 통과한 뒤 Inbox source 삭제와 Curation Queue 등록 같은 전환 산출물만 확인하는 단계. Evidence 내용의 보안 판단은 앞 단계 Receipt 결과를 입력으로 사용한다. |
| Publication Security Review | Evidence 생성 시 확정된 Data Protection Receipt, 마스킹, visibility와 발행 권한을 확인하는 발행 전 보안 Gate. |
| `needs_review` | 현재 단계가 적용 가능한 정책·절차와 근거만으로 자동 결론을 낼 수 없다는 단계별 판정 또는 결과. 그 자체로 Queue 상태나 사용자 대기를 뜻하지 않는다. |
| `awaiting_user` | 사용자만 제공할 수 있는 결정·입력·권한을 기다리는 Queue 상태. 현재 단계, 문의 사유, 요청 행동과 안전한 다음 행동을 함께 기록한다. |
| `review_handoff` | Curation이 Review 카드 생성과 참조 기록까지 완료했다는 결과. Review 카드의 reviewer는 사용자 또는 허용된 검증 Agent일 수 있으므로 `awaiting_user`와 같은 상태가 아니다. |
...
- 시스템 생성 대화·Outcome 텍스트는 `capture_conversation -> knowledge/inbox/<provider>/`까지만 동기 처리한다. Inbox 처리 규약의 첫 단계는 업무성 분류다. 별도 분류 입력에서 `non_business_confirmed`로 확정한 항목만 분류기·규칙 버전·사유를 남겨 `quarantine_inbox_item`으로 격리하여 Evidence·Curation 대상에서 제외하고, `list_inbox_disposals -> decide_inbox_disposal`의 일괄 검토에서만 복구 또는 폐기를 확정한다. 애매하거나 분류할 수 없는 항목은 분류 Receipt·격리·폐기 없이 `inspect_inbox -> review-data-protection -> accept_inbox -> ingest_accepted -> propose_pending` 일반 흐름을 따른다.
- **RB-ROUTE-008** 지식 변경은 `capture_* -> inspect_inbox -> review-data-protection -> accept_inbox -> ingest_accepted -> propose_update -> create_draft_bundle | apply_bundle_revision` 흐름을 사용한다. 자동 Curation은 대상 Evidence마다 `propose_update`를 내부 선행 단계로 실행하고 그 proposal을 Curator 입력·결정 근거로 유지한다. 수동 Curator도 자동 처리 목적의 Bundle 생성·갱신 전에는 같은 proposal을 먼저 읽는다. 저수준 `create-bundle`·revision 명령은 복구·명시적 수동 관리에 계속 사용할 수 있으나 자동 Curation의 proposal 단계를 우회하는 데 사용하지 않는다. `ingest_evidence`는 이 Gate를 통과한 후보를 변환하는 Runtime 내부 구현이며, CLI·MCP·Service의 직접 실행 경로가 아니다.
...
- **RB-ROUTE-011** Inbox 수집은 전체 저장소 테스트나 Bundle 정제를 실행하지 않는다. `inspect_inbox`는 읽기 전용, `review-data-protection`은 PII Scan·Agent의 선택적 민감정보 마스킹·근거 있는 민감성 결정을 하나의 `data_protection_receipt`로 확정하며, `accept_inbox`는 Receipt Gate, `ingest_accepted`는 Receipt·checksum·Evidence 생성 스키마와 전환 산출물 검증만 담당한다. `propose_pending`이 정제를 별도로 수행한다. 세부 전이·차단 사유·재처리는 등록된 Inbox 계약과 선택된 Profile을 따른다.
...
- **RB-ROUTE-015** `agent-rules/contracts/index.yaml`에 등록된 영역별 단계 계약은 현재 Frontmatter와 계약 작업 상태를 기준으로 안전한 선행 단계만 재수행할 수 있다. 계약은 승인, 민감성 판단, `needs_review` 뒤 안전 처리 결정을 추정하거나 대체하지 않는다.
- **RB-ROUTE-016** Curation 계약은 configured Curator의 분석 결과를 `no_bundle_recorded`, `review_handoff`, `published`, `draft_created`, 재시도 가능한 `queued` 중 하나로 기록한다. 각 outcome은 Queue 완료 또는 유지 여부를 함께 검증한다. Bundle 생성·갱신·승격의 경로와 Gate는 RB-CUR-001~006을 따른다.
- **RB-ROUTE-017** 작업은 동일 처리 주체가 연속으로 수행하는 단계들을 하나의 로컬 작업 단위로 묶는다. 아직 Commit하지 않은 같은 주체의 로컬 작업은 다음 작업과 함께 한 번에 공유할 수 있다. `agent -> user`, `user -> agent`, 또는 최종 종료 상태로 전환할 때만 **처리 주체·작업 상태 전이 Receipt**에 현재 결과·대상 checksum·다음 `next_action`·다음 처리 주체를 기록하고 Commit·Push한다. 이 Commit의 Push가 실패하면 해당 전이는 `publication_pending`이다. 다음 Push는 `resume-pending-push`로 현재 HEAD와 원격에 없는 모든 선행 Commit을 함께 전송하며, 성공 전에는 다음 처리 주체가 전이된 상태를 공유받은 것으로 보지 않는다.
- **RB-NOTIFY-001** 모든 사용자 알림은 `workspace/notifications/inbox/notification-<uuid>.json`의 `user-notification.schema.v1.json` 형식으로 한곳에 기록한다. 알림은 정본 작업 상태가 아니라 사용자용 요약 projection이며 `event`, `priority`, `title`, `summary`, `next_action`, `resource_ref`, `approval_required`, 관련 Evidence·Bundle·taxonomy revision과 `dedupe_key`만 기록한다. `bundle_created`, `taxonomy_change_proposed`, `reclassification_ready`, `review_requested`를 지원한다. `bundle_created`는 승인된 taxonomy에 따른 신규 Bundle 생성 사실만 알리고 `approval_required: false`다. `taxonomy_change_proposed`는 제안 경로·영향 Bundle 수를 요약하고 taxonomy 승인 요청을 표시한다. `reclassification_ready`는 영향 목록의 참조와 최신 revision을 제공하고, 사용자가 별도로 재분류를 명시 요청해야 한다는 `next_action`을 표시한다. `review_requested`는 기존 Curation Review의 사용자용 검토 요청 projection이다. 확인은 원본 알림을 바꾸지 않고 `workspace/notifications/acknowledgements/<notification-id>.json`에 기록한다. 정본 작업이 해결되거나 사용자가 알림을 철회하면 해당 inbox 알림과 동일 ID acknowledgement를 즉시 삭제한다. 결정·감사 근거는 Curation Review, taxonomy proposal, Bundle 또는 작업 receipt가 보존한다. 같은 대상·taxonomy revision·event의 중복 알림은 하나로 합치며, 메시지는 Evidence 원문·PII·자격증명·민감한 본문을 복사하지 않는다. 알림 전달 실패는 Bundle·taxonomy 변경을 되돌리거나 승인으로 간주하지 않으며, 영속 기록과 호출 결과의 `user_notification`을 남긴다. 외부 Connector 전달은 별도 설정·권한이 있을 때만 수행한다.
...
- **RB-KNW-027** `.circled-wiki/curation-taxonomy.yaml`은 설치별 Curation 분류 기준의 단일 기준이다. `schema_version: 1`은 명시적 `domains` 카탈로그(안전한 소문자 `id`, Agent가 분류 경계를 판단할 필수 `description`)와 `routing_rules`를 가진다. rule의 `domain`은 이 카탈로그에 등록돼야 하며, 각 rule은 모두 일치해야 하는 `match_terms`, 필수 `description`, 안전한 소문자 `bundle_type`, 필수 boolean `auto_create`, 선택적 ASCII `slug_prefix`를 가진다. `auto_create: true`는 기존 Bundle 후보 탐색과 의미 검증에서 일치 항목이 없고 단일 rule만 일치할 때 해당 domain/type의 신규 Draft 생성만 허용하며, 생성 이력과 사용자 알림을 Bundle에 남긴다. taxonomy 승인과 신규 domain·Bundle 생성은 기존 Bundle의 분류·경로·type을 바꾸지 않는다. Agent는 승인 대기 taxonomy 제안과 read-only 영향 목록을 `workspace/taxonomy-proposals/`에 자동 기록·알림할 수 있으나, 기존 domain·Bundle 재분류은 사용자가 해당 알림을 확인한 뒤 명시적으로 요청한 경우에만 실행한다. 적용은 acknowledgement된 `reclassification_ready` notification ID가 참조하는 영향 목록·제안 route, 승인된 taxonomy domain, read-only proposal의 현재 revision을 모두 대조하고 actor·rationale을 기록한 전용 작업으로만 허용하며 ID·UUID·Evidence 관계를 보존한다. Rule은 Review·Security·Publication Gate를 우회하지 않는다. 파일이 없으면 Agent는 관련 Bundle의 현재 분류를 조사해 사용자 승인용 초안만 만들며, 관례를 추정해 파일을 생성하지 않는다. Bootstrap·upgrade는 기존 taxonomy를 덮어쓰지 않는다.
- **RB-KNW-028** Runtime이 읽는 새 설치별 YAML 설정·정책·taxonomy를 만들거나 형식을 바꿀 때는 같은 변경에서 `.circled-wiki/schemas/<name>.schema.v<schema_version>.json` JSON Schema를 함께 만들거나 갱신한다. Schema는 `schema_version`, 허용·필수 필드, 타입, 고정 vocabulary와 구조 제약을 선언하고 `.circled-wiki/schemas/schema-registry.json` 및 `index.md`에 등록한다. Runtime loader는 Registry에서 YAML `schema_version`에 맞는 Schema를 선택해 파일별 기본값·호환성·보안 의미 검증 전에 공용 `validate_yaml_payload` 검증을 실행해야 하며, Schema가 없는 새 운영 YAML은 생성·배포·사용하지 않는다. Template, loader와 Schema의 변경은 함께 테스트하고 설치 루트에서 `circled-wiki validate-configuration`을 통과해야 한다. Markdown Frontmatter, 사람이 읽는 예시, test fixture와 Runtime이 읽지 않는 문서는 이 규칙의 대상이 아니다.
- **RB-KNW-029** YAML 데이터 계약의 버전은 해당 파일의 `schema_version`으로 관리하며, JSON Schema의 `$schema`(JSON Schema dialect)와 혼동하지 않는다. Schema의 `$id`에는 제품 계약 버전을 포함한다. 설명·오류 문구와 기존 유효 파일을 계속 허용하는 선택 필드 추가는 기존 버전을 유지할 수 있지만, 필드 삭제·이름 변경·타입 또는 의미 변경·기존 유효 파일을 무효로 만드는 제약 강화는 새 `schema_version`을 발급한다. 모든 지원 버전과 현재 버전은 `.circled-wiki/schemas/schema-registry.json`이 색인한다. 새 버전에서는 이전 버전 Schema를 보존하고 Runtime loader가 version별 Schema로 구조를 검증한 뒤 명시적 migration으로 최신 내부 모델로 변환한다. 지원 종료는 migration·설치 upgrade·release note와 함께 별도 변경으로만 수행하며, Schema 파일을 덮어써 과거 설치본의 검증 의미를 바꾸지 않는다.
- **RB-KNW-030** Bundle 리빌딩은 사용자의 명시 요청으로만 시작하는 협업 작업이다. 일반 Curation·생성·갱신은 자동 후보 탐지나 주기 통합을 수행하지 않는다. 사용자 요청이 있으면 Agent는 요청 대상을 조사하고 Evidence `source_ref.snapshot_at`, 없으면 `captured_at`의 최신순을 기준으로 통합·갱신·분리·archive를 한 번에 수행한다. 결과 Bundle을 먼저 검증하고 그 뒤 원본을 `knowledge/bundles/.archive/<domain>/`으로 이동하며, 결과·archive·복구 조건은 `workspace/bundle-rebuilds/` receipt에 자동 기록한다. domain·type·경로 이동에는 RB-KNW-027의 acknowledged reclassification Gate를 함께 적용한다.
...
- **RB-EVD-013** Evidence 수집 시 재사용 가치·보존 분류와 통합 Data Protection Receipt의 상태·정책 결정을 구분해 기록한다.
...
- **RB-EVD-020** Data Protection Review는 Inbox에서 Evidence로 넘길 후보에 PII Scan과 민감도 판단을 함께 적용하는 단일 단계다. PII Scan은 `.circled-wiki/data-protection.yaml`에서 `true`인 하드 마스킹 범주의 값을 실제 후보에서 기계적으로 마스킹한다. 민감도 판단 단계는 같은 활성 하드 범주를 다시 확인해 누락을 보완하고, 하드 마스킹 후 남은 실제 PII 후보와 Wiki의 `agent_mask_categories` 범주별 `include`·`exclude` 경계 및 업무 근거에 따라 추가 민감정보를 선택적으로 마스킹한다. 기본 Agent 범주는 고객 휴대전화·급여·평가·징계·미공개 사업정보·보안 구성과 명시적인 불법 행위 실행·조장·은폐 또는 타인의 권리·안전을 침해하는 구체적 지시로 한정하며, 정책에 선언되지 않은 구성원 프로필·입사·퇴사·업무 연락처는 변경하지 않는다. 계약·법률 자문·분쟁·소송·규제 대응과 그 결정은 법무 업무라는 이유만으로 제한하지 않는다. 명시적인 불법성이 확인되지 않은 법률·계약 내용은 `unlawful_content`로 추정하지 않는다. 최종 `source_checksum`·`candidate_checksum`·Scanner·버전·시각·결과·검토자·정책 판단을 하나의 `data_protection_receipt`에 기록한다. `passed` 또는 `masked` Receipt만 acceptance와 Evidence 변환을 허용하며 `needs_review`는 사용자만 안전 처리를 결정할 수 있을 때 `awaiting_user`로 유지한다. 본문·payload 또는 복사 메타데이터가 바뀌거나 재검토를 요청하면 Receipt 효력을 끝내고 통합 Data Protection 단계로 되돌린다. Inbox Review Queue의 식별·연관·상태 재개는 Intake ID UUID만 사용하며 checksum은 후보 변경 검출·Receipt 검증에만 사용한다.
- **RB-EVD-021** Capture 단계의 고위험 자격증명·식별자 차단은 원문 보존 전 안전장치이며 통합 Data Protection Review를 대체하지 않는다. Evidence 변환 단계의 Check는 통합 Receipt와 후보 checksum의 일치·Evidence 생성 스키마·전환 산출물에 한정하고, 보안 판단은 Receipt 결과를 입력으로 사용한다. 불변 원본이 필요한 파일은 원본을 자동 수정하지 않고 Git 비추적 제한 영역에 보존하며, 통합 Receipt가 `needs_review` 또는 `awaiting_user`인 입력은 Inbox와 Review Queue에 유지한다.
- **RB-EVD-023** Inbox 검토 완료부터 Evidence 확정과 `workspace/task/curation_reconciliation/<source_uuid>.md` 계약 작업 기록 등록까지를 하나의 원자 작업으로 처리한다. 생성된 Evidence는 이후 모든 흐름에서 읽고 참조만 하는 불변 기록이다. 작업 기록은 계약 이름·버전, `evidence_id`, Vault 루트 기준 `evidence/...` 경로인 `evidence_path`, 현재 단계·상태와 원문 없는 단계 Receipt만 가진다. Curation Queue는 이 중 `current.stage: queued`, `current.status: pending`인 작업의 조회 결과다. Curation이 Bundle을 만들거나 Review handoff를 만들면 작업 기록을 삭제한다. `no_bundle`, 소비된 Review, stale Review는 기록을 남기지 않고 삭제하며 stale Evidence는 다시 `queued`로 연다. Adapter 호출·계약 검증·결과물 생성 실패는 동일 작업 기록을 유지하고 마지막 차단 사유와 안전한 다음 행동만 갱신한다. 전체 스캔으로 누락된 작업 기록을 복구할 수 있어야 한다.
...
- **RB-SEC-001** 지원되는 PII 범주의 자동 처리는 `.circled-wiki/data-protection.yaml`의 `pii_scan.hard_mask_categories` 토글을 따른다. `true`인 주민등록번호·계좌번호·카드번호·자격증명(API key·token·password·private key 등)과 설정된 `agent_mask_categories`에 따라 마스킹한 값은 Bundle, Evidence, Task, Log, Prompt에 평문으로 기록하지 않으며, `false`인 범주는 하드 마스킹하지 않는다. `010`·`+82 10` 휴대전화와 이메일은 기능적으로 지원하고, 고객 휴대전화처럼 문맥이 필요한 대상은 `agent_mask_categories`에서 정확한 범위를 선언한다. 이미 마스킹된 값은 토글을 끄더라도 복원하지 않는다. `agent_mask_categories`에 선언되지 않은 구성원·협력업체 업무 연락처와 이메일은 내부 Receipt·Task·Log·Prompt·Evidence·Bundle에 업무상 필요한 경우 원문을 보존할 수 있으며, 이를 내부 운영 기록의 금지 항목으로 간주하지 않는다. 계약·법률 자문·분쟁·소송·규제 대응과 그 결정은 법무 업무라는 이유만으로 이 정책에서 제한하지 않으며, 명시적인 불법성이 확인되지 않은 법률·계약 내용도 자동 제한하지 않는다. 외부 전송·게시·발행에서는 별도 visibility·Publication Gate와 수신 목적에 따라 제한 또는 마스킹을 적용한다. 모든 Inbox 수집 주체는 공통 Capture 단계의 민감정보 사전 점검을 거치며, 통합 Data Protection Review가 활성 하드 범주 재확인·민감도 판단·최종 checksum을 하나의 Receipt에 기록한다. 실제 마스킹 대상의 처리 방식이 불명확할 때만 별도 사람 검토를 요청한다.
...
- **RB-SEC-005** Publication Security Review는 RB-EVD-020의 Evidence `data_protection_receipt` 계약 충족 여부와 visibility를 검증한다. Inbox에 남은 호환 projection만으로 Receipt를 대체하지 않는다.
- **RB-SEC-010** Evidence Ingest Agent의 보안 Check는 통합 Data Protection Review가 확정한 후보와 Receipt checksum의 일치·생성 스키마·전환 산출물에 한정한다. Receipt가 `needs_review` 또는 `awaiting_user`인 입력은 Inbox와 Review Queue에 유지한다.
...
```text
Evidence -> Curator -> 자동 Gate 또는 Review Gate -> Security Gate -> Commit
```
...
- **RB-PUB-003** 실패·미검토·`needs_review` 결과를 공식 지식으로 발행하거나 active Bundle로 승격하지 않는다. 다만 원문을 포함하지 않는 Inbox Review Queue·Curation Review·재시도 Receipt 같은 검증된 작업 상태 기록은 RB-PUB-013에 따라 Commit·Push하여 검토와 재처리를 가능하게 한다.
...
- **RB-PUB-013** Evidence 생성, Review Queue 생성·갱신, Curation Review·Draft 생성, Curation Queue의 재시도·차단 기록과 Bundle 발행은 유효한 상태 전이다. 같은 처리 주체가 연속 수행하는 상태 전이는 로컬 작업 단위로 묶을 수 있다. 처리 주체 변경 또는 최종 종료 전이만 PII·보안 Gate, Validator, 허용 경로·clean staging 검사를 통과한 뒤 configured publication policy에 따라 Commit·Push하여 다음 실행 주체가 볼 수 있게 한다. Curation Queue 완료가 포함되면 `workspace/task/curation_reconciliation/<uuid>.md` 삭제를 `verify-curation-commit`으로 확인한다. Bundle 변경에 필요한 provenance만 Bundle에 남기고, 완료된 Curation task와 Review 카드는 삭제한다. Push Receipt가 없으면 `publication_pending`으로 유지하며 다음 업무 단계로 진행하지 않는다. 의미적 최종 승인 여부와 Commit·Push 여부를 혼동하지 않는다. 보안 Gate의 `needs_review`는 실패가 아니라 `awaiting_user`이다.
...
accepted Evidence
  -> Curator
  -> runbook/manual: Curation Review 카드 -> 별도 검증 시도
  -> 사용자 명시 Review 요청: 요청 식별자가 기록된 Curation Review 카드
  -> 그 밖의 유형: 자동 Draft/갱신 Gate
  -> Security Receipt + 전체 Validator
  -> 전용 Promotion Gate로 active 전환
  -> 선택적 Git Commit / Push
```

- **RB-CUR-001** `runbook`과 `manual` 신규 Bundle 정제 제안은 Evidence checksum, 생성 actor와 제안 내용을 가진 `curation_review` 카드로 먼저 기록한다. `policy`, `guide`, `decision`, `spec`, `reference`, `report` Draft는 확정된 Evidence를 입력으로 직접 생성할 수 있다. Evidence 생성 시 확정된 Data Protection Receipt는 Curation에서 재실행하지 않는다.
...
- **RB-CUR-004** `runbook`·`manual` Review가 필요한 유형에서 카드가 없으면 Draft 생성·revision·Promotion을 중단한다. 운영 Agent의 자동 Curation은 이 두 유형 외에는 Review 카드를 생성할 수 없다. 단, 사용자가 명시적으로 요청한 경우에는 유형에 상관없이 요청 식별자(`user_review_request`)를 기록하고 Review 카드를 생성할 수 있다. Curation Queue 재처리는 동일 Evidence의 미완료 Review 카드를 먼저 찾아 재사용하고, 카드가 없을 때만 새 UUID 카드를 생성한다. Review 카드가 생성되면 Curator는 현재 요청 대화에서 승인 선택지를 묻지 않고 `knowledge/curation-reviews/`에 Review 카드 생성 결과를 기록하고 종료한다. `update_existing`는 대상으로 읽은 본문 checksum(`base_body_checksum`)과 변경 방식(`update_mode`)을 반드시 기록한다. `append`는 Runtime이 기존 본문을 보존한 채 delta를 추가하며 자동 갱신의 유일한 본문 변경 방식이다. 전체 본문 교체(`replace_full`)는 교체 사유가 있는 사용자·검토 경로에서만 허용한다. `runbook`·`manual` 승인 또는 `update_existing` 적용 직전에는 현재 Evidence checksum·대상 Bundle revision·기준 본문 checksum을 다시 확인하며, 달라지면 stale 처리한다. 그 밖의 기존 Bundle 갱신은 후보 Bundle의 `type`·도메인·태그·Evidence 참조 Frontmatter가 제안과 일치하고, 현재 Evidence의 `source_ref.snapshot_at` 또는 `captured_at`이 대상 Bundle의 최신 Evidence보다 같거나 최신이며, Security Receipt·대상 revision·전체 Validator·자동 갱신 Receipt를 통과할 때 자동 적용한다. 일치 후보가 없으면 신규 Bundle을 생성한다. 이 자동 Gate가 실패하면 Review로 바꾸지 않고 Curation Queue에 retryable blocker를 남긴다. Evidence checksum은 이미 확정된 Evidence의 동일성 확인이다. stale 카드는 삭제하고 해당 Evidence를 Curation Queue에 다시 등록해 현재 상태 기준 제안을 새로 만든다.
- **RB-CUR-005** `draft -> active` 상태 전환은 전용 Promotion Gate만 수행한다. 일반 Bundle 생성·revision API와 직접 Frontmatter 변경은 active 전환 수단이 아니다.
- **RB-CUR-006** 모든 active Bundle에는 승격 actor·시각, Security Receipt, 현재 Evidence checksum과 Evidence 생성 시 확정된 Data Protection Receipt를 provenance로 남긴다. `runbook`과 `manual`에는 생성 전 Review ID와 RB-CUR-003의 별도 검증 기록이 필수다. 최초 생성 Review와 이후 보완 Review는 `extensions.curation.review_receipts`에 Review ID·검증 시도 ID·검증 주체·시각·Evidence checksum·적용 revision·결정 메모로 누적 보존하며 최초 생성 Review를 덮어쓰지 않는다. `policy`, `guide`, `decision`, `spec`, `reference`, `report`는 확정된 Evidence·참조 무결성·전체 Validator Gate를 통과하면 자동으로 `active`로 승격할 수 있으며, provenance에 자동 승격임을 기록한다.
- **RB-CUR-007** 가상·테스트 Evidence도 같은 Gate를 적용한다. 테스트 목적이라는 이유로 필요한 Review, 별도 검증 시도 기록, 보안 검증 또는 Validator를 생략할 수 없다.
...
| Required Input 누락 | `needs_review` 결과로 누락 항목과 안전한 다음 행동을 기록. 입력이 새로 주어지면 새 준비 시도 또는 명시적 재검토로만 다시 시작 |
