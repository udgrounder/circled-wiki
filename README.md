...
이 저장소는 Campingtalk의 내부 지식과 이를 운영하는 AI Knowledge Operating System 설계를 함께 보관하는
Git 기반 기준 저장소다.
...
- 루트 Git 저장소 전체가 Source of Recovery다.
- 실제 지식 볼트는 `knowledge/` 폴더다.
- 설계 기준 문서는 `docs/` 폴더다.
- 전역 운영 불변식은 `OPERATING_RULES.md`, 작업별 실행 Profile은 `agent-rules/`이며 `docs/`는 Engineering Reference다.
- 구현체는 `src/knowledge_os/`에 두며 Core, CLI, MCP, worker의 책임을 분리한다.
- 저장소 문서, 설정, 예시는 절대 경로 대신 프로젝트 루트 상대 경로를 사용한다.
- `knowledge/bundles/`와 Evidence manifest, `.knowledge-os/templates/`, `.knowledge-os/schemas/`, `.knowledge-os/policies/`의 관리 문서는 YAML Frontmatter가 있는 OKF 구조를 유지한다.
...
`validated=<수> invalid=0`이면 현재 관리 문서가 OKF 최소 규칙과 Campingtalk Profile을 통과한 상태다.
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli ingest-evidence \
  --provider manual \
...
`.md` manifest가 생성된다. 처리 중에는 `.raw/`를 거치며, 10MiB를 넘는 원본은 Git에 넣지 않고
`.raw/`에 보존한다. 외부 저장소에 보관한 후 manifest를 별도로 처리해야 한다.
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli capture-conversation \
  --provider codex \
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli inspect-inbox --limit 100
PYTHONPATH=src python3 -m knowledge_os.cli accept-inbox \
  --intake inbox://campingtalk/codex/<intake-uuid> \
  --actor inspection-agent
PYTHONPATH=src python3 -m knowledge_os.cli ingest-accepted --limit 100
PYTHONPATH=src python3 -m knowledge_os.cli propose-pending --limit 100
```
...
파일은 기존 원본+manifest 방식을 유지한다. `--sensitivity-review` 기본값은 `required`이며 실제 검토가
...
일정 실행기는 `knowledge_os.integrations.collector.CollectedItem`과 `collect_items`를 사용해 전일 변경
목록을 provider별 `pending` Inbox 영수증으로 바꾼다. Notion API 호출·토큰 보관·실제 스케줄은 이 경계 밖의
Adapter가 담당한다.

```sh
PYTHONPATH=src python3 -m knowledge_os.cli capture-document \
  --provider notion \
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli propose-update \
  --evidence evidence://campingtalk/manual/2026/07/10/<source-uuid>
```

신규 공식 지식으로 만들기로 결정했으면 draft Bundle을 생성한다. `--evidence`에는 위에서 생성된 URI를
그대로 넣는다.

```sh
PYTHONPATH=src python3 -m knowledge_os.cli create-bundle \
  --domain cs \
  --slug refund-policy \
  --title "예약 환불 정책" \
  --type policy \
...
```sh
# 키워드 검색
PYTHONPATH=src python3 -m knowledge_os.cli search --query 환불

# 특정 종류만 검색
PYTHONPATH=src python3 -m knowledge_os.cli search --query 환불 --type policy

# Bundle 전문 읽기
PYTHONPATH=src python3 -m knowledge_os.cli read-bundle \
  --bundle knowledge://campingtalk/cs/refund-policy_<bundle-uuid>
```
...
운영자는 사용자 요청에 맞는 Workflow를 찾고 공식 지식과 분리된 runtime Task를 만든다.

```sh
PYTHONPATH=src python3 -m knowledge_os.cli find-workflow --request "이벤트 포스터를 만들고 싶어"

PYTHONPATH=src python3 -m knowledge_os.cli prepare-task \
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli prepare-runbook-refresh \
  --workflow poster-production \
...
```sh
PYTHONPATH=src python3 -m knowledge_os.cli record-outcome \
  --task <task-uuid> \
  --status completed \
...
`HERMES.md`를 먼저 읽고 따르도록 명시한다. `HERMES.md`는 `OPERATING_RULES.md`를 전역 운영 단일 규약으로 연결한다.
...
python -m pytest
PYTHONPATH=src python -m knowledge_os.cli validate
```
...
printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' \
  | PYTHONPATH=src .venv/bin/python -m knowledge_os.mcp.server
```
...
| `get_task` | Workflow Task 상태 조회 | 없음 |
| `update_task_inputs` | 누락된 Workflow 입력 보완 | `.runtime/` |
...
Hermes가 수집한 원본과 Workflow Outcome은 `knowledge/inbox/hermes/`에 적재한 후 같은 Inbox 검사·민감성 검토·승인·Evidence 변환을 거친다.
...
- `.raw/`와 `inbox/`는 Git에서 제외한다. 상세 보안 절차는 [Agent and Knowledge Security Policy](.knowledge-os/policies/agent-security.md)를 따른다.
...
- 외부 파일·바이너리: 보존 원본 + Markdown manifest
...
10MiB 이하 Evidence 원본은 manifest와 함께 Git에 추적한다. 10MiB를 초과하는 원본은 Git에서 제외하고
별도 원본 저장소에 보존하며 manifest만 추적한다.

MVP 검색은 OS 파일 검색, Frontmatter 필터, Markdown 링크 추적, Evidence manifest 검색으로 제공한다.

## 현재 구현된 MVP

- YAML Frontmatter 파싱과 OKF v0.1 최소 적합성 검증
- Campingtalk OKF Profile 검증과 Evidence/Bundle 참조 확인
...
- Git 지식과 분리된 `.runtime/tasks/` Task 상태
...
계획을 검토한 뒤에만 `--apply`를 붙인다. 운영 템플릿·스키마·시스템 정책은 모두 `.knowledge-os/` 아래에
...
그대로 보존한 채 새 버전을 `.knowledge-os/proposals/`에 제안본으로 둔다. 계획 보고서의 `backup_required`가
`true`이면 적용 전에 기존 `.knowledge-os/` 전체를 `.knowledge-os-backups/<기존-version>-<UTC timestamp>/`에
복사한다. 백업에 실패하면 기존 OS 파일을 쓰기 전에 업그레이드를 중단한다.

```sh
PYTHONPATH=src python3 -m knowledge_os.cli bootstrap-knowledge-os \
  --target /path/to/team-knowledge --apply
```
...
```sh
python3 .knowledge-os/bin/knowledge-os.py validate
python3 .knowledge-os/bin/knowledge-os.py search --query "환불 절차"
python3 .knowledge-os/bin/knowledge-os.py find-workflow --request "사용자 요청"
```

Python 3.9 이상과 `PyYAML`이 필요하다. AI Agent는 작업 시작 시
root `AGENTS.md` 또는 `CLAUDE.md`, `.knowledge-os/AGENT_BOOTSTRAP.md`, `.knowledge-os/OPERATING_RULES.md`, 그리고 요청에 맞는
`.knowledge-os/agent-rules/` Profile을 순서대로 읽고, 위 Launcher로 해당 프로젝트의 CLI를 호출한다.
Runtime과 Agent Bootstrap은 OS 관리 자산이므로 업그레이드 시 checksum·백업·충돌 제안 정책을 동일하게 적용한다.
`AGENTS.md`와 `CLAUDE.md`는 Agent 자동 발견용 참조 진입점이지만 OS 관리 자산은 아니다. 파일이 없으면 참조 전용으로 생성하고,
기존 파일에 `.knowledge-os/OPERATING_RULES.md` 참조가 없으면 표시된 Knowledge OS 참조 블록만 끝에 추가한다.
실제 운영 규칙·절차·명령은 `.knowledge-os/`에만 두며 기존 조직의 내용은 덮어쓰거나 OS manifest에 등록하지 않는다.

### 운영 이슈와 사용자 피드백

운영 중 발생한 오류·불편·개선 제안과 사용자가 제기한 문제는 `.knowledge-os/issues/`에 기록한다. CLI에서는
...
이슈 기록은 후속 시스템 개선 검토의 입력이며 자동 수정·정책 변경·발행을 수행하지 않는다. 상세 항목과 민감정보
제외 규칙은 `.knowledge-os/issues/README.md`와 `system-observation` Profile을 따른다.

지정된 System Maintainer는 사실을 바꾸지 않고 상태 이력만 추가한다. 표준 흐름은
`open -> triaged -> mitigated -> verified -> resolved`이며, `resolved`에는 독립 검증 근거가 필요하다.
...
`.knowledge-os/`를 별도 보존한 뒤 선택한 백업 폴더를 `.knowledge-os/`로 복원한다. 이 복구 과정에서도
`knowledge/`는 이동하거나 덮어쓰지 않는다. 복원 후 프로젝트에서 다음 검증을 실행한다.
