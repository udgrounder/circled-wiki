...
python3 .circled-wiki/bin/knowledge-os.py search --query "환불 절차"
...
`.circled-wiki/AGENT_BOOTSTRAP.md`, `.circled-wiki/OPERATING_RULES.md`, 그리고 요청에 맞는
...
기존 파일에 `.circled-wiki/OPERATING_RULES.md` 참조가 없으면 표시된 Circled Wiki 참조 블록만 끝에 추가한다.
...
운영 중 발생한 오류·불편·개선 제안과 사용자가 제기한 문제는 `.circled-wiki/issues/`에 기록한다. CLI에서는
...
제외 규칙은 `.circled-wiki/issues/README.md`와 `system-observation` Profile을 따른다.
