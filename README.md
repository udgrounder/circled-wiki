...
고정하고 `agent-rules/contracts/index.yaml`에 등록된 Inbox 계약에 따라 검사 통과한 항목만 자동 승인·Evidence 변환까지 진행한다. 결과는
`before`, 실행 action, `blocked` 항목으로 구분한다. 민감성·PII 판단 또는 미해결 Review Queue는 자동으로 해소하지 않고 유지한다.

`reconcile-curation --limit 100`은 `agent-rules/contracts/curation.yaml`에 따라 Curation Queue의 분석 결과를
...
각 배포 대상의 Receipt로 별도 관리하되, 제품 Issue 완료를 위해 모든 설치본을 개별 추적하지 않는다. 과거 운영 Issue에
당시 Receipt가 없으면, 사용자 검토 후 사용자가 지정한 대표 Wiki에서 증상이 재현되지 않는지 확인해 종료할 수 있으며,
이 경우 `processing.current_release_verification`에 현재 release ID, 검증자, 실행 명령과 결과를 기록한다.
